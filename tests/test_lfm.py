...
def test_get_training_corpus():
    ...
    class MockDataset:
        def take(self, n):
            return mock_dataset[:n]

    with patch("lfm_rus.tokenizer.load_dataset") as mock_load:
        mock_load.return_value = MockDataset()
        generator = get_training_corpus([{"path": "dummy"}], max_samples=3)
        # we will get all 3 in one batch since batch_size=1000
        batch1 = next(generator)
        assert batch1 == ["text1", "text2", "text3"]
